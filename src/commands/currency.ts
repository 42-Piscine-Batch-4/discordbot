...
export const execute = async (interaction: ChatInputCommandInteraction) => {
  ...
  // Add the input components to ActionRow
    const firstActionRow = new ActionRowBuilder<ModalActionRowComponentBuilder>()
      ...
      .addComponents(currencyFromInput);
		const thirdActionRow= new ActionRowBuilder<ModalActionRowComponentBuilder>()
      .addComponents(currencyToInput);
    ...
    const currencyFrom = response.fields.getTextInputValue('currencyFrom').toUpperCase();
    const currencyTo = response.fields.getTextInputValue('currencyTo').toUpperCase();
    ...
    if (error !== "") {
      ...
      const conversionAmt = await converter.convert(amount, currencyFrom, currencyTo);
      response.reply(
        { 
